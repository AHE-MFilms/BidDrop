...
function buildPage(firstName,lastName,email,phone){
  ...
  // ── Materials ──
  // Full catalog — enabledKey maps to acct.enabledMats flags set in Settings → Pricing
  const MATS=[
    {key:'1.3',name:'Architectural',   desc:'Most popular — dimensional look, 30-year warranty.',            mc:'#3b82f6',badge:'Most Popular',  bb:'rgba(59,130,246,.12)', bc:'#3b82f6', enabledKey:'arch'},
    {key:'1.5',name:'Impact-Resistant',desc:'Class 4 impact rating — qualifies for insurance discounts.',    mc:'#f59e0b',badge:'Impact Class 4', bb:'rgba(245,158,11,.12)',  bc:'#f59e0b', enabledKey:'impact'},
    {key:'1.8',name:'Designer Shingle',desc:'Premium curb appeal with enhanced impact resistance.',          mc:'#8b5cf6',badge:'Premium',        bb:'rgba(139,92,246,.12)', bc:'#8b5cf6', enabledKey:'des'},
    {key:'2.5',name:'Metal Roofing',   desc:'50+ year lifespan, energy efficient, maintenance-free.',       mc:'#10b981',badge:'Best Value',     bb:'rgba(16,185,129,.12)', bc:'#10b981', enabledKey:'metal'},
    {key:'0.9',name:'Flat / TPO',      desc:'Low-slope solution — durable, energy-efficient membrane.',     mc:'#64748b',badge:'Flat Roof',      bb:'rgba(100,116,139,.12)',bc:'#64748b', enabledKey:'flat'},
    {key:'3.2',name:'Tile / Clay',     desc:'Classic look, 50+ year lifespan, fire resistant.',             mc:'#ef4444',badge:'Tile',           bb:'rgba(239,68,68,.12)',  bc:'#ef4444', enabledKey:'tile'},
  ];
  ...
  if(est.priceOverride){
    ...
    // Filter: must have a price AND be enabled in account settings
    // acct.enabledMats is sent from the API; if absent (old API), fall back to price-only filter
    const em=acct.enabledMats||null;
    const avail=MATS.filter(m=>{
      if((_prices[m.key]||0)<=0) return false;  // no price computed = skip
      if(!em) return true;                       // old API: show all with a price
      return em[m.enabledKey]!==false;           // respect settings checkbox
    });
