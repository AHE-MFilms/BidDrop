...
function onMapClick(e){
  ...
  // or within 1.5s of completing a draw (the mouseup that finishes the rect fires as a map click)
  if(window._slDrawRect) return;
  if(window._slDrawCooldown) return;
  ...
  m.bindPopup(`
    <div style="min-width:200px;max-width:240px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-bottom:6px;">
        <button onclick="goEstFromPin('${pid}')" style="background:#F25C05;border:none;border-radius:7px;padding:9px 6px;color:white;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;">${pin.estimate && pin.estimate.total ? '📋 Edit Estimate' : '📋 Build Estimate'}</button>
        <button onclick="measureForPin('${pid}')" style="background:rgba(242,92,5,.1);border:1.5px solid #F25C05;border-radius:7px;padding:9px 6px;color:#F25C05;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;">📐 Measure Roof</button>
        <button onclick="openPhotoModal('${pid}')" style="background:rgba(34,197,94,.08);border:1.5px solid #22C55E;border-radius:7px;padding:9px 6px;color:#22C55E;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;">📷 Manage Photos</button>
        <button onclick="openNearbyCampaign('${pid}')" style="background:rgba(242,92,5,.08);border:1.5px solid #F25C05;border-radius:7px;padding:9px 6px;color:#F25C05;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;">📍 Nearby Campaign</button>
        <button onclick="openSendStormReport('${pid}')" style="background:rgba(96,165,250,.08);border:1.5px solid #60A5FA;border-radius:7px;padding:9px 6px;color:#60A5FA;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;">⚡ Storm Report</button>
        <div id="contact-info-${pid}">${pin.contactData ? buildContactInfoHTML(pid, pin.contactData) : (currentAccount && currentAccount.tracerfy_enabled ? `<button onclick="event.stopPropagation();lookupContactInfo('${pid}')" style="background:rgba(167,139,250,.08);border:1.5px solid #A78BFA;border-radius:7px;padding:9px 6px;color:#A78BFA;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;line-height:1.3;width:100%;height:100%;">📞 Get Contact</button>` : `<div style="font-size:9px;color:#6B7280;text-align:center;padding:4px;"></div>`)}</div>
      </div>
